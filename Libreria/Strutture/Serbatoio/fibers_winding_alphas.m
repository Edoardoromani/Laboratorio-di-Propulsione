...
alphas = linspace(0,54.7,100);
percentuals1 = zeros(1,length(alphas));
percentuals2 = zeros(1,length(alphas));
...
for i = 1:length(alphas)
    ...
    carbon.sigma = 3e9;             % Tensione massima ply UD direzione fibre (Pa)
    ...
    switch only_heli
        case "yes"
            ...
            talpha = talpha/carbon.volume_fraction;     % Spessore plies oblique con angolo alpha (mm)
            carbon.thick = talpha;  % Spessore totale carbon
        case "no"
            alpha = alphas(i); % Angolo delle plies oblique (elicoidali) (°)
            syms talpha thoop
    ...
    percentuals1(i) = carbon.talpha/(carbon.thoop+carbon.talpha);
    percentuals2(i) = carbon.thoop/(carbon.thoop+carbon.talpha);
end

figure
yyaxis left
plot(alphas,percentuals1)
ylabel("t_{alpha}%")
hold on
yyaxis right
plot(alphas,percentuals2)
ylabel("t_{hoop}%")
title("Percentage of helical over total thickness in composite fiber winding")
xlabel("alpha")