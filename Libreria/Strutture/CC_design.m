% CALCOLO SPESSORE CAMERA DI COMBUSTIONE IN COMPOSITO
...
p = 4; % Pressione interna serbatoio prevista (MPa)
f = 3; % Safety Factor
P = p*f; % Pressione di design serbatoio (MPa)
D = 146; % Diametro (interno) serbatoio (mm)

fibers_volume_fraction = 0.5; 
% Coefficiente rapporto volumetrico fibre/totale conservativo (si trova 0.54)

rho_fibers = 1800; % Densità fibre carbonio [Kg/m^3]
rho_matrix = 1200; % Densità matrice epossidica [Kg/m^3]
len_cyl = 0.3; % Lunghezza serbatoio [m]
...
E = 0.85; % Efficienza in letteratura

%% Thickness of the tank calculated with the small thickness approximation

metal.t1 = d*p*metal.FoS/(2*metal.Ys); % [m]
metal.t2 = d*p*metal.FoS/((2*metal.Ys*E-0.2*p)); % [m]

%% Verification that small thickness is allowed

metal.t = max(metal.t1,metal.t2); % smallest thickness needed
% metal.t = metal.t1;

if metal.t == metal.t1 
    disp("classic humble theory")
elseif metal.t == metal.t2
    disp("efficiency theory")
end
